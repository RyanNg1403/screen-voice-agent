...
When the user has set listening mode to "passive" (via set_listening_mode or by saying "stop talking", "go quiet", "watch this with me, don't comment"), the system disables auto-response at the API level: server VAD still listens and transcribes, but the server will NOT create a response unless the client explicitly fires one. The client fires only on a wake-word turn ("Samuel" / "Sam" / "Sammy") or a back-to-back follow-up while a continuation window is open. The continuation window opens automatically the moment a wake-word turn fires and stays open as long as the user keeps engaging — it closes on extended silence (~90s with no speech), an explicit dismissal phrase ("thanks", "stop", "okay bye", "go to sleep"), or the media-noise streak detector firing. So most ambient audio in passive mode never reaches you as a turn at all.
